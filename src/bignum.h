...
// OpenSSL 1.1 made BIGNUM and BN_CTX opaque types and removed BN_init().  This
// header was originally written against the pre-1.1 API where CBigNum derived
// directly from BIGNUM.  It is reimplemented here on top of the public,
// OpenSSL 3.0-compatible accessor API: CBigNum now *owns* a heap-allocated
// BIGNUM* rather than inheriting from BIGNUM.  All consensus-relevant
// serialization (getvch/setvch, GetCompact/SetCompact, getuint256, ...) is
...
public:
    ...
    // Access to the underlying OpenSSL BIGNUM, for the few call sites that
    // hand a CBigNum straight to a raw OpenSSL function.  Deliberately NOT an
    // implicit conversion operator: that would make expressions like
    // `bn < 0` ambiguous (CBigNum-vs-CBigNum comparison vs. pointer-vs-int).
    ...
    */
    ...
    {
        BN_bin2bn(&vchBytes[0], vchBytes.size(), bn);
    }
    ...
    */
    bool isPrime(const int checks=BN_prime_checks) const {
        (void)checks; // OpenSSL 3 BN_check_prime selects the iteration count itself
        CAutoBN_CTX pctx;
