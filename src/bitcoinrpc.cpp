...
Value stop(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop HoboNickels server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "HoboNickels server stopping";
}
...
{ //  name                      function                 safemd  unlocked  isWalletFn
  //  ------------------------  -----------------------  ------  --------  ----------
    ...
    { "createsnapshot",         &createsnapshot,         true,   false,    false },
    { "getbestblockhash",       &getbestblockhash,       true,   false,    false },
    { "getconnectioncount",     &getconnectioncount,     true,   false,    false },
    { "getpeerinfo",            &getpeerinfo,            true,   false,    false },
    { "ping",                   &ping,                   true,   false,    false },
    { "getnettotals",           &getnettotals,           true,   true,     false },
    { "getnetworkinfo",         &getnetworkinfo,         true,   true,     false },
    { "uptime",                 &uptime,                 true,   true,     false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: HoboNickels-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
bool ClientAllowed(const boost::asio::ip::address& address)
{
    ...
    // (the obsolete IPv4-compatible form ::a.b.c.d is no longer handled).
    if (address.is_v6()
     && address.to_v6().is_v4_mapped())
        return ClientAllowed(
            asio::ip::make_address_v4(asio::ip::v4_mapped, address.to_v6()));
...
public:
    ...
    bool connect(const std::string& server, const std::string& port)
    {
        ...
        auto endpoints = resolver.resolve(server, port, error);
        if (error)
            return false;
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params, CWallet* pWallet) const
{
    ...
    std::shared_ptr<CWallet> spWallet;
        if (!pWallet)
        {
            try
            {
